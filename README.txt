0. (!) Временный пользовательский интерфейс проекта до разработки фронтенда: 127.0.0.1:8000/
Данная страница будет использоваться для регистрации пользователей/подключения пуш-уведомлений.
...
4. Аутентификация / регистрация: allauth.
    ...
    - Настроена возможность рассылки писем (с яндекс аккаунта, в дальнейшем будет создан специальный тестовый аккаунт для
    проекта), в т.ч. при регистрации.
...
9. Подключение отложенных задач: реализовано через signals и apscheduler (как альтернатива Celery)
    ...
    - Пуш-уведомления: реализованы на основе Django-Webpush и сигналов (см. privatechat/signals.py и utube_app/signals.py)
    ...
    Пуш-уведомления отправляются:
     ...
     После ввода кода поле phone_verified в модели CustomUser меняется на True, что разблокирует дополнительные возможности
     для пользователя: стать автором, менять аватары и т.д.